# B3RB ROS 2 Package (`b3rb_ros_line_follower`)

This ROS 2 package serves as your starter template for programming a simulated ambulance buggy.
Your goal is to safely navigate a track, avoid static obstacles, identify traffic sign boards, detect patients and hospitals by scanning QR codes, interact with a server, and park at the final destination.
...
| `/sign_board_detection` | `std_msgs/msg/String` | Subscribed (by `runner`) | Decoded labels of detected turn signs. |
...
| `/ServerCommunication` | `synapse_msgs/msg/ServerCommunication` | Bidirectional (Sub & Pub by `runner`) | Interface with the server. |
...
Use this to report your progress and receive targets:
...
*   `msg` (`string`): The payload content
...
2.  **Obstacle Avoidance (`b3rb_ros_line_follower.py` / `lidar_callback`)**:
    ...
    *   If the minimum front distance drops below threshold, override lane steering, steer around the obstacle, and merge back once the scan is clear.
